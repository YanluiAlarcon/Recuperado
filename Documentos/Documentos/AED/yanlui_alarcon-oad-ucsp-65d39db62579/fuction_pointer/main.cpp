...
void Selection(int *first, int size, int (*compara) (int,int)){
    int i,j,t,menor;
    for(i = 0; i < size ; i++){
        for(j = i, menor = i ; j < size ; j++){
            if(compara(first[j] , first[menor]) < 0)
                menor = j;
        }
        t = first[i];
        first[i] = first[menor];
...
void Insertion(int *first, int size, int (*compara) (int,int)){
    for (int i = 0; i < size - 1; i++){
        int j = i + 1;
        int tmp = first[j];
        if(compara(first[j],first[j-1]) < 0){
...
void Cocktail(int *first, int size , int (*compara)(int,int)) {
